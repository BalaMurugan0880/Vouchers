...
class travelController extends Controller
{
       function test_query(){
   	 ...
   	 return view('/voucherCategory/travel',['data' => $data]);
   }
    ...
    public function reedemVoucher(Request $request){
      ...
      if ($vendortype == 'basicuserPackage') {
          if ($voucherRedeem >= 3 ) {
             return redirect('travel')->with('message', trans('Upgrade To Advance Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
                
                         return redirect('travel')->with('message', trans('Already Redeem This Voucher'));
...

             {
                 ...
                 DB::table('addvouchers')->where('id', $id)->increment('is_redeemed', 1);
                         ...
                         $uservoucher->save();
                         return redirect('travel')->with('message', trans('Voucher Redeemed Successfully'));
             }
          }
          
      }elseif ($vendortype == 'advanceuserPackage') {
        if ($voucherRedeem >= 5 ) {
             return redirect('travel')->with('message', trans('Upgrade To Premium Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
        ...
        if ($voucherRedeem >= 1 ) {
             return redirect('travel')->with('message', trans('Upgrade To Basic Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
