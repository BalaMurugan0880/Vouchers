...
class fashionController extends Controller
{
       function test_query(Request $request){
   	 ...
   	 return view('voucherCategory/fashion',['data' => $data]);
   }
      ...
      if ($vendortype == 'basicuserPackage') {
          if ($voucherRedeem >= 3 ) {
             return redirect('fashion')->with('message', trans('Upgrade To Advance Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
                
                         return redirect('fashion')->with('message', trans('Already Redeem This Voucher'));
...

             {
                  ...
                  DB::table('addvouchers')->where('id', $id)->increment('is_redeemed', 1);
                         ...
                         $uservoucher->save();
                         return redirect('fashion')->with('message', trans('Voucher Redeemed Successfully'));
             }
          }
          
      }elseif ($vendortype == 'advanceuserPackage') {
        if ($voucherRedeem >= 5 ) {
             return redirect('fashion')->with('message', trans('Upgrade To Premium Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
        ...
        if ($voucherRedeem >= 1 ) {
             return redirect('fashion')->with('message', trans('Upgrade To Basic Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
