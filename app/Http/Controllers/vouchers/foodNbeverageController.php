...
class foodNbeverageController extends Controller
{
       function test_query(){
   	 ...
   	 $data = DB::table('companyregisters')
            ...
            ->where(['companyregisters.category' => 'Food&Beverage'])
   	 		->get();

   	 

   	 return view('voucherCategory/foodNbeverage',['data' => $data]);
   }
      ...
      if ($vendortype == 'basicuserPackage') {
          if ($voucherRedeem >= 3 ) {
             return redirect('foodNbeverage')->with('message', trans('Upgrade To Advance Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
                
                         return redirect('foodNbeverage')->with('message', trans('Already Redeem This Voucher'));
...

             {
                  ...
                  DB::table('addvouchers')->where('id', $id)->increment('is_redeemed', 1);
                         ...
                         $uservoucher->save();
                         return redirect('foodNbeverage')->with('message', trans('Voucher Redeemed Successfully'));
             }
          }
          
      }elseif ($vendortype == 'advanceuserPackage') {
        if ($voucherRedeem >= 5 ) {
             return redirect('foodNbeverage')->with('message', trans('Upgrade To Premium Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
        ...
        if ($voucherRedeem >= 1 ) {
             return redirect('foodNbeverage')->with('message', trans('Upgrade To Basic Package To Redeem More Vouchers'));
          }else{
               if ($redeemer)  
             {
